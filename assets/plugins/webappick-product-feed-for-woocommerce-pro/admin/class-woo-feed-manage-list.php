...
class Woo_Feed_Manage_list extends Woo_Feed_List_Table
{
    ...
    function column_option_name($item)
    {
        ...
        $actions = array(
            ...
            'duplicate' => sprintf('<a href="?page=%s&action=%s&feed=%s&_wpnonce=%s" class="single-feed-duplicate" style="cursor: pointer;">' . __('Duplicate', 'woo-feed') . '</a>', esc_attr($_REQUEST['page']), 'duplicate-feed', $item['option_name'], $duplicate_nonce)
        );
    ...
    public function process_bulk_action()
    {
        ...
        if ('duplicate-feed' === $this->current_action()) {
            ...
            if (!wp_verify_nonce($nonce, 'wf_duplicate_feed')) {
                update_option('wpf_message', 'Failed To Duplicate Feed. You do not have sufficient permission to duplicate.');
                wp_redirect(admin_url("admin.php?page=woo_feed_manage_feed&wpf_message=error"));
            } else {
                global $wpdb;
                $var = $_REQUEST['feed'];
                ...
                $result = $wpdb->get_results($query, 'ARRAY_A');
                $count = $wpdb->num_rows;
                $baseFeed = unserialize(get_option($var));
                $newFeedName = $var . "-" . "Copy-" . $count;

                # Copy Feed Configuration
                $newFeedRules = array();
                if (count($baseFeed['feedrules'])) {
                    foreach ($baseFeed['feedrules'] as $key => $value) {
                        if ($key == 'filename') {
                            $newFeedRules['feedrules'][$key] = str_replace("wf_feed_","",$newFeedName);
                        } else {
                            $newFeedRules['feedrules'][$key] = $value;
                        }
                    }
                    $newFeedRules['url'] = str_replace($baseFeed['feedrules']['filename'] . "." . $baseFeed['feedrules']['feedType'], str_replace("wf_feed_","",$newFeedName) . "." . $baseFeed['feedrules']['feedType'], $baseFeed['url']);
                    $newFeedRules['last_updated'] = date("Y-m-d H:i:s");
                }

                # Copy Feed FIle
                $upload_dir = wp_upload_dir();
                $base = $upload_dir['basedir'];
                $path = $base . "/woo-feed/" . $baseFeed['feedrules']['provider'] . "/" . $baseFeed['feedrules']['feedType'];
                $file = $path . "/" . $baseFeed['feedrules']['filename'] . "." . $baseFeed['feedrules']['feedType'];
                $newFile = $path . "/" . str_replace("wf_feed_","",$newFeedName) . "." . $baseFeed['feedrules']['feedType'];
                copy($file, $newFile);

                # Save Duplicated feed
                update_option($newFeedName, serialize($newFeedRules));
                update_option('wpf_message', 'Feed Duplicated Successfully');
                wp_redirect(admin_url("admin.php?page=woo_feed_manage_feed&wpf_message=success"));
            }
        }
