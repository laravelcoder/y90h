...
//        wp_register_style($this->woo_feed, plugin_dir_url(__FILE__) . 'css/select2.min.css', array(), $this->version, 'all');
...
//        wp_register_script($this->woo_feed . "_wf_select2", plugin_dir_url(__FILE__) . 'js/select2.min.js', array(), $this->version, false);
//        wp_enqueue_script($this->woo_feed . "_wf_select2");
    ...
    public function load_admin_pages()
    {
        ...
        if (function_exists('add_options_page')) {
            ...
            if (get_option('wpfp_activated') == 'Activated') {
                add_submenu_page(__FILE__, __('Manage Attributes', 'woo-feed'), __('Manage Attributes', 'woo-feed'), 'manage_options', 'woo_feed_manage_attribute', 'woo_feed_manage_attribute');
                add_submenu_page(__FILE__, __('Category Mapping', 'woo-feed'), __('Category Mapping', 'woo-feed'), 'manage_options', 'woo_feed_category_mapping', 'woo_feed_category_mapping');
                add_submenu_page(__FILE__, __('WP Options', 'woo-feed'), __('WP Options', 'woo-feed'), 'manage_options', 'woo_feed_wp_options', 'woo_feed_wp_options');
            }
        }
    }
}
